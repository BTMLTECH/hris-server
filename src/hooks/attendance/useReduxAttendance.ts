...
export const useReduxAttendance = (): AttendanceContextType => {
  ...
  const { attendanceCache, attendancePagination } = useAppSelector(
  ...
  const {
    data: adminAttendanceReport,
    isLoading: adminReportLoading,
    error: adminReportError,
  } = useAdminAttendanceReportQuery(undefined, {
    skip: !user || !user.role || user.role !== "admin",
  });

  const {
    data: exportedAttendanceData,
    isLoading: exportLoading,
    error: exportError,
  } = useExportAttendanceExcelQuery(undefined, {
    skip: !user || !user.role || user.role !== "admin",
  });

  const [biometryCheckIn, { isLoading: biometryCheckInLoading }] =
    useBiometryCheckInMutation();
  const [biometryCheckOut, { isLoading: biometryCheckOutLoading }] =
    useBiometryCheckOutMutation();
  const [manualCheckIn, { isLoading: manualCheckInLoading }] =
    useManualCheckInMutation();
  const [manualCheckOut, { isLoading: manualCheckOutLoading }] =
    useManualCheckOutMutation();
