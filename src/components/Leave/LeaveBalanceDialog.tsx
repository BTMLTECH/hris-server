...
import { LEAVE_TYPES, LeaveTypeKey } from "@/utils/leaveType";
...
export function LeaveBalanceDialog({
  ...
  const handleTypeChange = (type: LeaveTypeKey) => {
  setSelectedType(type);
  ...
  const balances = selectedEmployee.leaveBalance?.balances as Record<LeaveTypeKey, number> | undefined;

  // Dynamic lookup
  const val = balances?.[type] ?? 0;
  ...
  {employees.length === 0 ? (
    ...
    <Table className="min-w-[900px]">
      ...
      <TableBody>
        {employees.map((emp) => (
          <TableRow key={emp._id}>
            ...
            <TableCell className="font-medium">
              {reverseDepartmentMap[emp?.department]}
            </TableCell>

            {LEAVE_TYPES.map(({ key, label }) => {
              const balance =
                emp.leaveBalance?.balances?.[key as LeaveTypeKey] ?? 0;

              return (
                <TableCell key={key} className="text-center">
                  <Badge variant={getBadgeVariant(balance)}>
                    {balance}
                  </Badge>
      ...
      <div>
        ...
        <select
          value={selectedType}
          onChange={(e) => handleTypeChange(e.target.value as LeaveTypeKey)}
          className="w-full border rounded-md px-3 py-2 text-sm"
        >
          {LEAVE_TYPES.map(({ key, label }) => (
