...
const calculateDays = (start: string, end: string) => {
  ...
  formData.relievers?.forEach(reliever => formDataToSend.append("relievers", reliever));
  ...
  const handleApproveLeaveRequestFlow = async (requestId: string, action: string) => {
  ...
  const handleRejectLeaveRequestWithNote = async (id: string, note: string, action: string): Promise<boolean> => {
                  ...
                  {/* {request.status === "pending" && (
                    ...
                    <div className="flex space-x-2">
                      <Button
                        ...
                        onClick={() => handleApproveLeaveRequestFlow(request.id, 'approved')}
                      >
  ...
  >
    <DialogContent>
      ...
      <DialogFooter>
        ...
        >
          {isLocalLoading(selectedRequest?.id, 'reject') && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          Reject Leave
        </Button>
      </DialogFooter>
    </DialogContent>
