...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, ResponsiveContainer, PieChart, Pie, Cell, ComposedChart, AreaChart, Area, Line } from 'recharts';
import { TrendingUp, TrendingDown, Users, Calendar, DollarSign, Award, Activity, PieChart as PieChartIcon } from 'lucide-react';
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1">Detailed insights and key metrics for your organization.</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 lg:gap-6">
        <Card className="flex flex-col">
          ...
          <CardContent>
            <div className="text-lg lg:text-2xl font-bold">{analytics?.keyMetrics?.employeeGrowth?.value || '0'}%</div>
            <p className="text-xs text-muted-foreground">{analytics?.keyMetrics?.employeeGrowth?.trend || 'vs last quarter'}</p>
          </CardContent>
        </Card>
    <Card className="flex flex-col">
  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
  ...
  <CardContent>
    ...
    <p className="text-xs text-muted-foreground">
      {analytics?.keyMetrics?.avgSalary?.trend || "0"}
        ...
        <Card className="flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs lg:text-sm font-medium">Total Employees</CardTitle>
            <Users className="h-3 w-3 lg:h-4 lg:w-4 text-gray-600" />
          </CardHeader>
          <CardContent>
            <div className="text-lg lg:text-2xl font-bold">{analytics?.dashboardCards?.totalEmployees?.value || '0'}</div>
            <p className="text-xs text-muted-foreground">{analytics?.dashboardCards?.totalEmployees?.trend || 'based on current data'}</p>
          </CardContent>
        </Card>
        <Card className="flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-lg lg:text-2xl font-bold">{analytics?.keyMetrics?.leaveUtilization?.value || '0'}%</div>
            <p className="text-xs text-muted-foreground">{analytics?.keyMetrics?.leaveUtilization?.trend || 'of allocated leave'}</p>
          </CardContent>
        </Card>
        <Card className="flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs lg:text-sm font-medium">Appraisals Due</CardTitle>
            <Award className="h-3 w-3 lg:h-4 lg:w-4 text-yellow-500" />
          </CardHeader>
          <CardContent>
            <div className="text-lg lg:text-2xl font-bold">{analytics?.dashboardCards?.appraisalsDue?.value || '0'}</div>
            <p className="text-xs text-muted-foreground">{analytics?.dashboardCards?.appraisalsDue?.trend || 'last 30 days'}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-sm">New hires vs. employee terminations over time.</CardDescription>
          </CardHeader>
          <CardContent className="min-h-[300px]">
            {hiringTrendsData.length > 0 ? (
              <ChartContainer config={chartConfig}>
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={hiringTrendsData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="month" fontSize={12} />
                    <YAxis fontSize={12} />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Bar dataKey="hires" fill="#10B981" name="New Hires" radius={[4, 4, 0, 0]} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-sm">Average attendance percentage over time.</CardDescription>
          </CardHeader>
          <CardContent className="min-h-[300px]">
              ...
              <ChartContainer config={chartConfig}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={attendanceData}>
                    ...
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Area type="monotone" dataKey="attendance" stroke="#8B5CF6" fill="#8B5CF6" name="Attendance" />
                  </AreaChart>
                </ResponsiveContainer>
              </ChartContainer>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-sm">Compare average salaries across departments.</CardDescription>
          </CardHeader>
          <CardContent className="min-h-[300px]">
              ...
              <ChartContainer config={chartConfig}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={salaryByDeptData}>
                    ...
                    <YAxis fontSize={12} />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Bar dataKey="avgSalary" fill="#F59E0B" name="Avg Salary" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </ChartContainer>
            ) : (
              ...
              <ChartContainer config={chartConfig} className="h-full">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={leaveTypesData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <YAxis fontSize={12} />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Bar dataKey="total" fill="#E5E7EB" name="Total Allocated" radius={[4, 4, 0, 0]} />
                  ...
                  <PieChart>
                    ...
                    >
                      {salaryByDeptData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={`hsl(${Math.random() * 360}, 70%, 50%)`} />
                      ))}
                    </Pie>
                    <ChartTooltip content={<ChartTooltipContent />} />
                  </PieChart>
