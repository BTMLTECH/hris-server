...
<form onSubmit={handleSubmit} className="space-y-5">
  ...
  <div>
    ...
    <Input
      id="trainer"
      type="text"
      value={trainingFormData?.trainer || `${currentUser.firstName} ${currentUser.lastName}`}      
      required
    />
  </div>
        ...
        <div className="space-y-2">
          {cachedEmployees
            ...
            .map((emp: ProfileFormData) => {
              const isChecked =
                trainingFormData?.participantEmails?.includes(emp.email);
              return (
                <div key={emp._id} className="flex items-center space-x-2">
                  <Checkbox
                    checked={isChecked}
                    onCheckedChange={(checked) => {
                      let updated = trainingFormData?.participantEmails || [];
                      if (checked) {
