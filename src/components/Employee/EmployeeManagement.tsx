...
const handleCompany = async () => {
  ...
  return (
    ...
    <SelectTrigger className="w-48">
      ...
      <SelectContent>
        ...
        <SelectItem value="roaghi">Regional Office - Airport & GHI</SelectItem>
            ...
            {[...filteredEmployees]
              ...
              .map((employee: ProfileFormData) => (
                <TableRow key={employee._id}>
                  ...
                  <TableCell>
                    <span className="capitalize">{employee.staffId || '-'}</span>
                  </TableCell>
                  {/* Employee Info */}
                  <TableCell>
                    <div className="flex items-center space-x-3">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {canManageEmployees && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleViewDetails(employee, 'view')}
 ...
 {employee.status === 'active' && (
                ...
                <Button
                  ...
                  onClick={() => {
                    dispatch(setSelectedActionId(employee._id));
                    dispatch(setSelectedActionType('terminate'));
                    dispatch(setIsActionDialogOpen(true));
                  }}
                  disabled={isLocalLoading(employee._id, 'terminate')}
                >
  ...
  <Button
    ...
    disabled={isLocalLoading(employee._id, 'activate')}
  >
    {isLocalLoading(employee._id, 'activate') ? (
      ...
      <UserCheck className="h-4 w-4" />
                       ...
                       {(employee.sendInvite || employee.resetRequested || !employee.isActive) && (
                            <Button
                              variant="outline"
                              size="sm"
                              className="flex items-center gap-2 text-sm"
                              onClick={() =>
                                handleResendInvite(employee.email, employee._id, 'resend')
                              }
                              disabled={isLocalLoading(employee._id, 'resend')}
                            >
